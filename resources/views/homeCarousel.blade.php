		...
		<li class="featureItem">
			...
			<div class="carouselDescription">
				...
				<div class="infoContainer">
					...
					<ul class="buttonList">
						<li>
							<button class="like" onclick="likeFeature(event, {{ $feature->feature->id }}, true)">{{ Auth::user()->featureLikes()->where('feature_id', $feature->feature->id)->first() ? "Liked" : "Like" }}</button>
							<div class="likes">{{ $feature->feature->likes}}</div>
						</li>
						<li>
							<button class="going" onclick="goingFeature(event, {{ $feature->id }}, true)">{{ Auth::user()->featureGoings()->where('queue_id', $feature->id)->first() ? "Going!" : "Going" }}</button>
							<div class="goings">{{ $feature->going }}</div>
						</li>
					</ul>
				</div>
			</div>
